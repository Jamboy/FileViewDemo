...
public class SuperFileView2 extends FrameLayout implements TbsReaderView.ReaderCallback {
    ...
    public void createSaveFile(){
        String downSaveFilePath = "/storage/emulated/0/Download/com.jambo.download";
        File downSaveFile =new File(downSaveFilePath);

        if (!downSaveFile.exists()) {
            TLog.d("准备创建/storage/emulated/0/Download/com.jambo.download！！");
            boolean mkdir = downSaveFile.mkdir();
            if(!mkdir){
                TLog.e("创建/storage/emulated/0/Download/com.jambo.download失败！！！！！");
//                return false;
            }
        }
//        return true;
